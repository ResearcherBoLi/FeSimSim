...
char *INPdomodel(CKTcircuit *ckt, struct card *image, INPtables * tab)
{
...
#endif
    /*  --------  Check if model is a memristor --------- */ 
	else if (strcmp(type_name, "memr") == 0){
		err = INPfindLev(line, &lev);
		switch (lev){
			case 1:
				type = INPtypelook("MEMR1");
				if (type < 0) {
				err =
				    INPmkTemp
				    ("Device type HP Ion Linear Drift Memrsitor Model not available in this binary\n");
			    }
			    break;
			case 2:
			    type = INPtypelook("MEMR2");
			    if (type < 0) {
				err =
				    INPmkTemp
				    ("Device type VTEAM Memristor model not available in this binary\n");
			    }
			    break;			
			case 3:
			    type = INPtypelook("MEMR3");
			    if (type < 0) {
				err =
				    INPmkTemp
				    ("Device type Diffuse Memristor Model not available in this binary\n");
			    }
			    break;
			case 4:
			    type = INPtypelook("MEMR4");
			    if (type < 0) {
				err =
				    INPmkTemp
				    ("Device type Simense Tunnelling Barrier Memristor model  not available in this binary\n");
			    }
			    break;
    ...
    /*  --------  Check if model is a memcapacitor --------- */ 
	else if (strcmp(type_name, "memc") == 0){
		err = INPfindLev(line, &lev);
		switch (lev){
			case 1:
				type = INPtypelook("MEMC1");
				if (type < 0) {
				err =
				    INPmkTemp
				    ("Such Ideal Memcapacitor Model not available in this binary\n");
			    }
			    break;
	...
	else if (strcmp(type_name, "memi") == 0){
		err = INPfindLev(line, &lev);
		switch (lev){
			case 1:
				type = INPtypelook("MEMI1");
				if (type < 0) {
				err =
				    INPmkTemp
				    ("Such Ideal Meminductor Model not available in this binary\n");
			    }
			    break;
    ...
    /*  --------  Check if model is a fecap --------- */ 
	else if (strcmp(type_name, "fecap") == 0){
		err = INPfindLev(line, &lev);
		switch (lev){
			case 1:
				type = INPtypelook("FECAP1");
				if (type < 0) {
				err =
				    INPmkTemp
				    ("LK FeCap Model not available in this binary\n");
			    }
			    break;
			case 2:
			    type = INPtypelook("FECAP2");
			    if (type < 0) {
				err =
				    INPmkTemp
				    ("Preisach FeCap model not available in this binary\n");
			    }
			    break;			
			case 3:
			    type = INPtypelook("FECAP3");
			    if (type < 0) {
				err =
				    INPmkTemp
				    ("Reduced LK FeCap model Device type Diffuse Memristor Model not available in this binary\n");
			    }
			    break;
